...
if (!file.exists(fit_file)) {
  fit <- covidseir::fit_seir(
    ...
    iter = ITER,
    chains = CHAINS,
    start_decline_prior = c(log(get_google_start("Florida", dat)), 0.1), # c(log(start_decline), 0.2),
    end_decline_prior = c(log(get_google_end("Florida", dat)), 0.1), # c(log(end_decline), 0.2),
