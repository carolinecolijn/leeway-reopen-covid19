...
ratios %>%
  group_by(region) %>%
  summarise(
    q0.05 = quantile(ratio, probs = 0.05),
    q0.5 = median(ratio),
    q0.95 = quantile(ratio, probs = 0.95)) %>%
  ggplot(aes(x = region, y = q0.5, ymin = q0.05, ymax = q0.95)) +
  geom_pointrange() +
  coord_flip() +
...
# mults <- c(0.3, 0.3 * 1.2, 0.3 * 1.4, 0.3 * 1.6, 0.999)
...
out <- filter(out, !is.na(frac)) # 1 extra date
