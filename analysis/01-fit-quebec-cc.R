...
f_qc = make_f_seg(dat,.date= "2020-04-20")
day_ch = which(dat$date==lubridate::ymd("2020-05-20"))
day_ch2 = which(dat$date==lubridate::ymd("2020-06-15"))

f_qc[seq(day_ch, length(f_qc))] <- 3
f_qc[seq(day_ch2, length(f_qc))] <- 4
data.frame(date=dat$date, fseg = f_qc)
...
# Check fit -----------------------------------------------------------------
 ...
 proj <- covidseir::project_seir(fit, iter = 200:400, forecast_days = 60)
